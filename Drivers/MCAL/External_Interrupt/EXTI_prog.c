...
static volatile void *EXTI_ApvoidGenaricParameter[3]={NULL,NULL,NULL};
...
ES_t EXTI_enuCall_Back(volatile void (*Copy_PtoFunAPP)(void *),volatile void *Copy_PvoidParamenter,u8 Copy_u8Interrupt_ID)
{
	...
	if(Copy_PtoFunAPP != NULL)
	{
		if(Copy_u8Interrupt_ID<3)
		{
			...
			EXTI_ApvoidGenaricParameter[Copy_u8Interrupt_ID]=Copy_PvoidParamenter;
		}
		else
		{
			Local_u8ErrorState = ES_OUT_OF_RANGE;
...
ISR(INT0)
{
	if(PtoFunc[0] != NULL)
	{
		PtoFunc[0](EXTI_ApvoidGenaricParameter[0]);
	}
}
ISR(INT1)
{
	if(PtoFunc[1] != NULL)
	{
		PtoFunc[1](EXTI_ApvoidGenaricParameter[1]);
	}
}
ISR(INT2)
{
	if(PtoFunc[2] != NULL)
	{
		PtoFunc[2](EXTI_ApvoidGenaricParameter[2]);
	}
